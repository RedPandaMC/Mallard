...
- **Dashboard layout**: restructured to match the design handoff: KPI cards, then
  the spend gauge beside the restriction banner (two-column `wv-gauge-row` grid),
  then the filter bar, then analysis controls, then the charts grid with a section
  label. Standalone model-list section removed (data is in the models chart panel).
