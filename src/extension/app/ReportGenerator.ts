...
import { formatCredits, formatMoney, formatTokens } from '../domain/format';
...
<title>Mallard Usage Report — ${esc(rangeLabel)}</title>
<style>
...
<div class="footer">
  Generated by Mallard · ${esc(generatedAt)} · Source: ${esc(s.source)}
</div>
</body>
</html>`;
}
...
export function defaultReportPath(): string {
  ...
  return path.join(os.homedir(), 'Downloads', `mallard-report-${month}.html`);
}
