"""Tests for InfluxDB write helper — uses a mock async write_api."""
...
def _api_capturing(captured: list) -> MagicMock:
    """A mock async write API whose write() records the Point and returns True."""
    api = MagicMock()
    api.write = AsyncMock(side_effect=lambda **kw: captured.append(kw["record"]))
    return api


class TestWritePayload:
    async def test_write_called_once(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import write_payload

        api = MagicMock()
        api.write = AsyncMock(return_value=True)
        await write_payload(api, bucket="metrics", org="mallard", metric=sample_metric)
        assert api.write.call_count == 1

    async def test_write_receives_correct_bucket_and_org(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import write_payload

        api = MagicMock()
        api.write = AsyncMock(return_value=True)
        await write_payload(api, bucket="my-bucket", org="my-org", metric=sample_metric)

        _, kwargs = api.write.call_args
        assert kwargs["bucket"] == "my-bucket"
        assert kwargs["org"] == "my-org"

    async def test_point_measurement_name(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import _MEASUREMENT, write_payload

        captured: list = []
        await write_payload(_api_capturing(captured), bucket="metrics", org="mallard", metric=sample_metric)

        assert len(captured) == 1
        assert captured[0]._name == _MEASUREMENT

    async def test_point_contains_instance_id_and_connector_tags(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import write_payload
    ...
    async def test_missing_instance_id_and_connector_tagged_unknown(self) -> None:
        from server.influx import write_payload

        metric = NormalizedMetric(schema_version=1, instance_id=None, ts_ms=1_700_000_000_000, connector=None)
        captured: list = []
        await write_payload(_api_capturing(captured), bucket="metrics", org="mallard", metric=metric)

        point = captured[0]
        assert point._tags.get("instance_id") == "unknown"
        assert point._tags.get("connector") == "unknown"

    async def test_point_contains_numeric_fields(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import write_payload
    ...
    async def test_none_fields_are_omitted_not_zeroed(self) -> None:
        from server.influx import write_payload
    ...
    async def test_extra_fields_stored_as_json(self) -> None:
        from server.influx import write_payload
    ...
    async def test_counter_maps_expand_to_prefixed_fields(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import write_payload
    ...
    async def test_map_field_keys_are_sanitised(self) -> None:
        from server.influx import write_payload
    ...
    async def test_active_models_stored_as_one_joined_field(self, sample_metric: NormalizedMetric) -> None:
    ...
    async def test_no_extra_json_field_when_extra_is_empty(self, sample_metric: NormalizedMetric) -> None:
        from server.influx import write_payload
    ...
    async def test_null_top_model_stored_as_empty_string(self) -> None:
        from server.influx import write_payload
        ...
        assert captured[0]._fields["top_model"] == ""
...
class TestMakeClient:
    def test_make_client_passes_url_token_org(self) -> None:
        ...
        with patch("server.influx.InfluxDBClientAsync") as MockClient:
...
class TestPingInflux:
    async def test_ping_returns_true_on_success(self) -> None:
        ...
        assert await ping_influx(mock_client) is True
    ...
    async def test_ping_returns_false_on_exception(self) -> None:
        ...
        assert await ping_influx(mock_client) is False
